...
public:
    ...
    void setup_minimizer(parallel::thread_id_t thread, stats::Tracked& minimizer);
    void setup_initializer(parallel::thread_id_t thread, stats::Tracked& initializer);
    void setup_objective(parallel::thread_id_t thread, stats::Tracked& objective);
    void setup_postproc(parallel::thread_id_t thread, stats::Tracked& objective);
    std::unique_ptr<ResultStatsGatherer> create_results_gatherer(parallel::thread_id_t thread, const std::shared_ptr<const TrainingSpec>& spec);
...
private:
    ...
    void add_accu(std::string key, parallel::thread_id_t thread, const std::shared_ptr<stats::StatisticsCollection>& accumulator);
